...
double FindH(double h, double alpha, double x, double y, double& z) /*€ вроде понимаю как это работает*/
{
	double k1, k2, k3, k4;
	double l1, l2, l3, l4;
...
double RungeKutteMethod(double alpha, double& max_error, bool printT)
{
	...
	while (x < 1) { // тк рекурсивно вычил€ем, а именно каждый следующий эллемент считаетс€ через предыдущий
		double h = 0.2;
		double y1, y2, y3;
		double z1;
		y_prev = y;
...
void shooting_metod()
{
	...
	do
	{
		...
		max_error = -1; // тк передает значение по ссылке нужно после каждой итерации его "обнул€ть"
	} while (abs(y-B)>eps);
